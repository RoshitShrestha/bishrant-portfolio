...
function handleResize() {
  ...
  peel.handleDrag(function (_, x, y) {
    ...
    // Create a DOMMatrix from the transform (identity if 'none')
    const matrix = new DOMMatrix(transform === "none" ? undefined : transform);

    // Invert the matrix to convert screen coordinates to element-local coordinates
    const inverse = matrix.inverse();

    // Map mouse coordinates into the element's local space
    const localPoint = inverse.transformPoint(new DOMPoint(x, y));

    // Save current position
    currentPos.x = localPoint.x;
    currentPos.y = localPoint.y;
